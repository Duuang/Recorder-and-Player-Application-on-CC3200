...
//
//
//CC3200热点SSID：cc3200-cky，无密码。
//有人网络助手：TCP client模式，IP地址：192.168.1.1，端口：5001
//工程大部分代码为WLAN_AP下的代码，删除了部分冗余函数。
//
//*****************************************************************************
...
//
//利用TCP控制板载LED的亮灭。
//主要函数为ControlLED(),利用了TCP通信过程的各个函数。
//CC3200热点SSID：cc3200，无密码。
//有人网络助手：TCP client模式，IP地址：192.168.1.1，端口：5001
//工程大部分代码为WLAN_AP下的代码，删除了部分冗余函数。
//
//*****************************************************************************
...
const char pcSsidName[33]="cc3200-cky";
...
//****************************************************************************
void ControlLED(unsigned long ulIpAddr);
...
{  
	...
	char g_cBsdBuf[800];
	// filling the buffer
	memset(g_cBsdBuf,-1,sizeof(g_cBsdBuf));
	...
	Report("Listend to the port: 5001. \r\n");
	// putting the socket for listening to the incoming TCP connection
	iStatus = sl_Listen(iSockID, 0);

	UART_PRINT("Ready to accept connection. \r\n");


	iNewSockID = sl_Accept(iSockID, ( struct SlSockAddr_t *)&sAddr,
			(SlSocklen_t*)&iAddrSize);
	UART_PRINT("Connection is established.\r\n");
	//
	//draw standby page
	//
	fillScreen_a();
	drawString("Standby",0,100,5,GREEN);
	drawString("  >>>",0,180,5,GREEN);

	while (1)
	{
		iStatus = sl_Recv(iNewSockID, g_cBsdBuf, iTestBufLen, 0);

		if(g_cBsdBuf[0]=='1')   // start recording
		{
			GPIOPinWrite(GPIOA1_BASE, 0x2, 0x2);
			fillScreen_a();  //draw starting page
			drawString("Starting",0,100,4,YELLOW);
			drawString("  ...",0,180,5,YELLOW);

			UART_PRINT("recorder will start in a moment... \n\r");
			long lRetVal;
			unsigned char policyVal;
			long lFileHandle;
			unsigned long ulToken;
			if(RecordAndSave(&ulToken, &lFileHandle) < 0)  //jump to recorder
			    {
			...
			//stanby page
			fillScreen_a();
			drawString("operation",0,100,4,GREEN);
			drawString(" success!",0,180,4,GREEN);
			osi_Sleep(1000);
			fillScreen_a();
			drawString("Standby",0,100,5,GREEN);
			drawString("  >>>",0,180,5,GREEN);
		...
		if(g_cBsdBuf[0]=='2')  // start sending file to PC via UART
		{
			GPIOPinWrite(GPIOA1_BASE, 0x2, 0);
			//UART_PRINT("RED LED OFF. \n\r");
			//draw sending
			fillScreen_a();
			drawString("Sending",0,100,4,RED);
			drawString("> > >",0,180,5,RED);
			long lRetVal;
			unsigned char policyVal;
			long lFileHandle;
			unsigned long ulToken;
		...
		if(g_cBsdBuf[0]=='3')
		{
			...
			fillScreen_a();
			drawTraingle(200, 100, 100, 100, 150, 200, GREEN);

		}
		if(g_cBsdBuf[0]=='4')
		{
			GPIOPinWrite(GPIOA1_BASE, 0x4, 0);
			UART_PRINT("ORANGE LED OFF. \n\r");
			fillScreen_a();
			fillRectangle(100, 100, 100, 100, RED);
		}
		if(g_cBsdBuf[0]=='5')
		{
